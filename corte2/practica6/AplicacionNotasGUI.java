...
public class AplicacionNotasGUI extends JFrame {
    ...
    private JScrollPane crearPanelTabla() {
        String[] columnas = {"Codigo", "Nombre", "Programa", "Jornada", "Nota 1", "Nota 2", "Nota 3", "Definitiva", "Estado"};
    ...
    private void actualizarResumen() {
        ...
        for (int i = 0; i < total; i++) {
            double definitiva = Double.parseDouble(modeloTabla.getValueAt(i, 7).toString().replace(",", "."));
            sumaDefinitivas += definitiva;
            if (modeloTabla.getValueAt(i, 8).toString().equals("Aprueba")) {
                aprueban++;
            } else {
                reprueban++;
            }
        ...
        double promedio = (total > 0) ? sumaDefinitivas / total : 0.0;
        lblResumen.setText(String.format(Locale.US, "Registros: %d | Promedio: %.2f | Aprueban: %d | Reprueban: %d", total, promedio, aprueban, reprueban));
    }
        ...
        if (selector.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            try (PrintWriter salida = new PrintWriter(archivo, "UTF-8")) {
                for (int col = 0; col < modeloTabla.getColumnCount(); col++) {
                    salida.print(modeloTabla.getColumnName(col) + (col == modeloTabla.getColumnCount() - 1 ? "" : ";"));
                }
                salida.println();
                for (int fila = 0; fila < modeloTabla.getRowCount(); fila++) {
                    for (int col = 0; col < modeloTabla.getColumnCount(); col++) {
                        salida.print(modeloTabla.getValueAt(fila, col) + (col == modeloTabla.getColumnCount() - 1 ? "" : ";"));
                    }
                    salida.println();
                }
    ...
    private void salirAplicacion() {
        if (JOptionPane.showConfirmDialog(this, "Desea salir de la aplicacion?", "Confirmar salida", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
            System.exit(0);
        }
    }
